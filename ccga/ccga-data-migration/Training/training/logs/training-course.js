...
const migrate = async d => {
    ...
    const [rows, fields] = await conn.execute(
        `select sc.*, sct.name as type_name from sar_course sc inner join sar_course_type sct on sct.id = sc.sctid and sc.closed = 1`
    );
...
const processEachRow = async (conn, row, d) => {
        ...
        const trainingFormType = "trainingcourse";
    const [rows, fields] = await conn.query(
        'select u.email, u.id, scp.status FROM sar_course_person_xref scp inner join sar_person sp on sp.id = scp.spid inner join `users` u on u.id = sp.uid where scp.scid = ?',
        [row.id]
    );
